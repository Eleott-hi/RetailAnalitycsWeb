...
export default function ItemList({ t_name, columns, items }: { t_name: string, columns: any[string], items: any[] }) {
    ...
    const updateTable = () => apiGetTableAsync(t_name).then(([table, _]) => setTable(table)).catch(error => console.error(error));
    const deleteTable = () => apiDeleteTableAsync(t_name).then(updateTable).catch(error => console.error(error));
